...
sudo modprobe wlan_mt76x8_usb then dmesg -w: expect probe on 04e8:20b1 interface 2, ROM patch (mt7668_patch_e*_hdr.bin) + RAM code download, wlan0 appears.
ip link show wlan0 up; sudo iw dev wlan0 scan | grep SSID returns neighborhood networks.
Connect to the user's AP via NetworkManager (nmcli dev wifi connect ...), ping test.
Replug the dongle → auto-rebind. Reboot → module autoloads.
Smoke-test a /proc/wlan/dbgLevel read (validates the proc_ops conversion at runtime).
Confirm btusb can still claim interfaces 0–1 (BT is out of scope; the new_id trick from the user's notes applies unchanged).
